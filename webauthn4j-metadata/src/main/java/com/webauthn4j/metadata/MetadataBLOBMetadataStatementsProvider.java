...
public class MetadataBLOBMetadataStatementsProvider implements MetadataStatementsProvider{

    private final MetadataBLOBProvider metadataBLOBProvider;

    public MetadataBLOBMetadataStatementsProvider(MetadataBLOBProvider metadataBLOBProvider) {
    ...
    public Map<AAGUID, Set<MetadataStatement>> provide() {
        return metadataBLOBProvider.provide().getPayload().getEntries().stream()
                .collect(Collectors.toMap(
                        ...
                        entry -> Collections.singleton(entry.getMetadataStatement())
                ));
    }
}
